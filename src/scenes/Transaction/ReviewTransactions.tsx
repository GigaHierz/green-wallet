...
function ReviewTransactions() {
  ...
  const [txServiceUrl, setTransactionServiceUrl] = useState<string>(
    CHAIN_INFO[DEFAULT_CHAIN_ID].transactionServiceUrl
  );
  ...
  useEffect(() => {
    async function getPendingTransactions() {
      ...
      const safeService = new SafeApiKit({
        txServiceUrl: updatedTxServiceUrl,
        ethAdapter,
      });
      console.log({ safeAddress, safeService });
      const pendingTransactionsResults = (
        ...
        <tbody>
          {pendingTransactions.map((transaction) => (
            <tr key={transaction.hash}>
              <td>
                <a
                  href={`${txServiceUrl}/api/v1/multisig-transactions/${transaction.safeTxHash}`}
                  target="_blank"
                  rel="noreferrer"
                >
              ...
              <td>
                <button
                  ...
                  onClick={(event) =>
                    confirmTransacton(event, transaction.safeTxHash)
                  }
                >
                  Confirm
                </button>
